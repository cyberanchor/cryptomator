#!/usr/bin/env python3
"""
Crypt-Argon2 - A cross-platform file encryption/decryption tool (OOP fork with Argon2id, v2.6).
...
- Optimized for large files (>4 GB) with stream-based hashing and I/O.
...
WARNING: Version 2.6 is not compatible with files encrypted by versions <2.3 due to
...
- psutil (pip install psutil, for memory detection)
...
Usage:
    ...
    python crypt.argon2.py --dry-run --encrypt --folder ./path --password mypass --mmap
...
class CryptConfig:
    ...
    BUFFER_SIZE_LOW: int = 512 * 1024  # 512 KB buffer for low-memory systems
    BUFFER_SIZE_DEFAULT: int = 1024 * 1024  # 1 MB buffer for default systems
    LARGE_FILE_THRESHOLD: int = 1024 * 1024 * 1024  # 1 GB threshold for large files
    MMAP_THRESHOLD: int = 4 * 1024 * 1024 * 1024  # 4 GB threshold for mmap
...
class CryptKeyManager:
    ...
    def compute_file_hash(self, file_path: Path, buffer_size: int, use_mmap: bool = False) -> bytes:
        """
        Compute SHA-512 hash of a file's contents using streaming for large files.

        Args:
            file_path: Path to the file.
            buffer_size: Size of the read buffer (bytes).
            use_mmap: If True, use mmap for large files (if supported).

        Returns:
            bytes: 64-byte SHA-512 hash.

        Raises:
            ValueError: If file cannot be read or exceeds system limits.
        """
        self.logger.debug(f"Computing SHA-512 hash for: {file_path}, buffer_size={buffer_size}, use_mmap={use_mmap}")
        start_time = time.time()
        try:
            file_size = os.stat(file_path).st_size
            if file_size > sys.maxsize:
                raise ValueError(f"File {file_path} size ({file_size} bytes) exceeds system limit ({sys.maxsize} bytes)")
            self.logger.debug(f"File size: {file_size} bytes")

            sha512 = hashlib.sha512()
            if use_mmap and file_size >= self.config.MMAP_THRESHOLD:
                try:
                    with open(file_path, 'rb') as f:
                        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                            sha512.update(mm)
                    self.logger.debug(f"Hashed {file_path} using mmap")
                except (ValueError, OSError) as e:
                    self.logger.warning(f"mmap failed for {file_path}: {e}, falling back to streaming")
                    use_mmap = False
            ...
            self.logger.info(f"Computed SHA-512 hash for {file_path} in {elapsed_time:.2f}s: {hash_value.hex()[:16]}...")
            return hash_value
...
class CryptFileProcessor:
    ...
    def __init__(self, config: CryptConfig, low_memory: bool = False, verbose: bool = False):
        ...
        self.low_memory = low_memory or (self.available_memory < 8 * 1024 * 1024 * 1024)  # <8 GB
        self.buffer_size = (
            self.config.BUFFER_SIZE_LOW if self.available_memory < 8 * 1024 * 1024 * 1024
            else self.config.BUFFER_SIZE_DEFAULT
        )
        self.key_manager = CryptKeyManager(config, self.low_memory)
        self.logger.debug(
            f"Initialized CryptFileProcessor: low_memory={self.low_memory}, "
            f"verbose={verbose}, available_memory={self.available_memory} bytes, "
            f"buffer_size={self.buffer_size} bytes"
        )

    def _get_available_memory(self) -> int:
        """
        Get available system memory using psutil or default to 4 GB if unavailable.
        ...
        if PSUTIL_AVAILABLE:
            try:
                ...
                self.logger.warning(f"Failed to get available memory: {e}, defaulting to 4 GB")
        return 4 * 1024 * 1024 * 1024  # Default to 4 GB
    ...
    def _read_file(self, file_path: Path, use_mmap: bool = False) -> bytes:
        """
        Read file content with streaming for large files and permission checks.
        ...
        try:
            ...
            file_size = os.stat(file_path).st_size
            self.logger.debug(f"Reading {file_path}, size={file_size} bytes, use_mmap={use_mmap}")

            if file_size > self.config.LARGE_FILE_THRESHOLD and use_mmap and self.available_memory >= 16 * 1024 * 1024 * 1024:
                try:
                    with open(file_path, 'rb') as f:
                        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                            data = mm.read()
                    self.logger.debug(f"Read {len(data)} bytes from {file_path} using mmap")
                    if self.verbose:
                        print(f"{Fore.CYAN}Read {len(data)} bytes from {file_path} using mmap{Style.RESET_ALL}")
                    return data
                except (ValueError, OSError) as e:
                    self.logger.warning(f"mmap failed for {file_path}: {e}, falling back to streaming")
            ...
            with open(file_path, 'rb') as f:
                while True:
                    ...
                    data.extend(chunk)
            data = bytes(data)
            self.logger.debug(f"Read {len(data)} bytes from {file_path} using streaming")
            if self.verbose:
                print(f"{Fore.CYAN}Read {len(data)} bytes from {file_path}{Style.RESET_ALL}")
            return data
        except (IOError, PermissionError, MemoryError, OSError) as e:
            self.logger.error(f"Failed to read file {file_path}: {e}")
            raise ValueError(f"Error reading file {file_path}: {e}")

    def _write_file(self, file_path: Path, data: Union[bytes, Iterator[bytes]]) -> None:
        """
        Write data to a file with streaming for large files and permission checks.

        Args:
            file_path: Path to write the file.
            data: Data to write (bytes or iterator of byte chunks).
            ...
            with open(file_path, 'wb') as f:
                if isinstance(data, bytes):
                    for i in range(0, len(data), self.buffer_size):
                        f.write(data[i:i + self.buffer_size])
                else:
                    for chunk in data:
                        f.write(chunk)
            self.logger.debug(f"Wrote data to {file_path}")
            if self.verbose:
                print(f"{Fore.CYAN}Wrote data to {file_path}{Style.RESET_ALL}")
        except (IOError, PermissionError, MemoryError, OSError) as e:
            self.logger.error(f"Failed to write file {file_path}: {e}")
            raise ValueError(f"Error writing file {file_path}: {e}")

    def encrypt_file(self, input_file: str, output_dir: str, password: str, debug: bool = False, dry_run: bool = False, use_mmap: bool = False) -> None:
        """
        ...
        Args:
            ...
            use_mmap: If True, use mmap for large files.
        ...
        try:
            ...
            if file_size > sys.maxsize:
                raise ValueError(f"File {input_path} size ({file_size} bytes) exceeds system limit ({sys.maxsize} bytes)")

            input_hash = self.key_manager.compute_file_hash(input_path, self.buffer_size, use_mmap)
            if self.verbose:
                print(f"{Fore.CYAN}Input SHA-512 hash: {input_hash.hex()[:32]}...{Style.RESET_ALL}")

            data = self._read_file(input_path, use_mmap)
    ...
    def decrypt_file(self, encrypted_file: str, output_dir: str, password: str, debug: bool = False, dry_run: bool = False, use_mmap: bool = False) -> None:
        """
        ...
        try:
            ...
            self._validate_password(password)
            file_size = os.stat(encrypted_path).st_size
            if file_size < self.config.SYSTEM_BLOCK_LENGTH:
                raise ValueError(
                    f"File {encrypted_path} too short ({file_size} bytes, expected at least {self.config.SYSTEM_BLOCK_LENGTH})"
                )
            if file_size > sys.maxsize:
                raise ValueError(f"File {encrypted_path} size ({file_size} bytes) exceeds system limit ({sys.maxsize} bytes)")

            data = self._read_file(encrypted_path, use_mmap)
            ...
            computed_hash = self.key_manager.compute_file_hash(temp_path, self.buffer_size, use_mmap)
    ...
    def analyze_file(self, file_path: str, debug: bool = False, use_mmap: bool = False) -> None:
        """
        ...
        try:
            ...
            if file_size < self.config.SYSTEM_BLOCK_LENGTH:
                raise ValueError(
                    f"File {file_path} too short ({file_size} bytes, expected at least {self.config.SYSTEM_BLOCK_LENGTH})"
                )
            if file_size > sys.maxsize:
                raise ValueError(f"File {file_path} size ({file_size} bytes) exceeds system limit ({sys.maxsize} bytes)")

            data = self._read_file(file_path, use_mmap)
            if data[:len(self.config.SIGNATURE)] == self.config.OLD_SIGNATURE:
                raise ValueError(
    ...
    def process_folder(
        ...
        single_file: Optional[str] = None, debug: bool = False, dry_run: bool = False, use_mmap: bool = False
    ) -> None:
        """
        ...
        try:
            ...
            if single_file:
                ...
                if not file_path.is_file():
                    ...
                    return
                if mode == 'encrypt':
                    self.encrypt_file(str(file_path), str(output_dir), password, debug, dry_run, use_mmap)
                    processed_files += 1
                elif mode == 'decrypt':
                    try:
                        with open(file_path, 'rb') as f:
                        ...
                        if signature == self.config.SIGNATURE:
                            self.decrypt_file(str(file_path), str(output_dir), password, debug, dry_run, use_mmap)
                            processed_files += 1
                ...
                for file in folder_path.glob('*'):
                    if file.is_file():
                        if mode == 'encrypt':
                            self.encrypt_file(str(file), str(output_dir), password, debug, dry_run, use_mmap)
                            processed_files += 1
                        else:
                            try:
                                with open(file, 'rb') as f:
                                    signature = f.read(len(self.config.SIGNATURE))
                                if signature == self.config.SIGNATURE:
                                    self.decrypt_file(str(file), str(output_dir), password, debug, dry_run, use_mmap)
                                    processed_files += 1
                                elif signature == self.config.OLD_SIGNATURE:
                                    self.logger.error(
...
class CryptCLI:
    ...
    def __init__(self):
        ...
        try:
            ...
            psutil_version = pkg_resources.get_distribution("psutil").version if PSUTIL_AVAILABLE else "not installed"
        except pkg_resources.DistributionNotFound as e:
            self.logger.error(f"Failed to get dependency versions: {e}")
            crypto_version = argon2_version = colorama_version = psutil_version = "unknown"
        self.logger.info(
            f"Starting {PROGRAM_NAME} v{PROGRAM_VERSION}, "
            f"dependencies: cryptography={crypto_version}, argon2-cffi={argon2_version}, "
            f"colorama={colorama_version}, psutil={psutil_version}"
        )
    ...
    def _read_password_from_file(self, file_path: str) -> str:
        ...
        try:
            ...
            return password
        except (IOError, PermissionError, UnicodeDecodeError, OSError) as e:
    ...
    def _print_startup_info(self, args: argparse.Namespace) -> None:
        ...
        memory_mode = '64 MB' if args.low_memory or (CryptFileProcessor(self.config).available_memory < 8 * 1024 * 1024 * 1024) else '4 GB'
        print(f"Argon2id: memory={memory_mode}, iterations={self.config.ARGON2_ITERATIONS}, parallelism={self.config.ARGON2_PARALLELISM}")
        print(f"Buffer size: {self.config.BUFFER_SIZE_LOW if memory_mode == '64 MB' else self.config.BUFFER_SIZE_DEFAULT} bytes")
        print(f"Verbose: {args.verbose}, Debug: {args.debug}, Dry run: {args.dry_run}, Mmap: {args.mmap}")
        print(f"System: {'64-bit' if sys.maxsize > 2**32 else '32-bit'} Python, max file size: {sys.maxsize} bytes")
    ...
    def run(self) -> None:
        ...
        parser = argparse.ArgumentParser(
            description=(
                ...
                "Optimized for large files (>4 GB) with streaming and optional mmap.\n"
            ...
            epilog=(
                ...
                f"  Large file with mmap: python crypt.argon2.py --encrypt --file ./large.bin --password mypass --mmap"
            ),
        ...
        parser.add_argument('--mmap', action='store_true', help='Use mmap for large files (>4 GB, requires sufficient memory)')
        ...
        if args.password and args.password_file:
            ...
            return
        if args.mmap and not args.file and not args.folder:
            print(f"{Fore.RED}Error: --mmap requires --file or --folder{Style.RESET_ALL}")
            return
        if args.mmap and args.available_memory < 16 * 1024 * 1024 * 1024:
            print(f"{Fore.YELLOW}Warning: --mmap requires at least 16 GB of available memory, ignoring mmap{Style.RESET_ALL}")
            args.mmap = False
        ...
        if args.analyze:
            ...
            processor.analyze_file(args.file, args.debug, args.mmap)
            return
        ...
        processor.process_folder(
            ...
            Path(args.file).name if args.file else None, args.debug, args.dry_run, args.mmap
        )
